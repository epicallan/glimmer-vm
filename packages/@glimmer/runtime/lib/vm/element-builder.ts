...
export class NewElementBuilder implements ElementBuilder {
  ...
  static forInitialRender(doc: Simple.Document, cursor: Cursor) {
    let builder = new this(doc);
    builder.initializeCursor(cursor.element, cursor.nextSibling);
    return builder;
  }
  ...
  static resume(doc: Simple.Document, tracker: Tracker, nextSibling: Option<Simple.Node>) {
    ...
    let stack = new this(doc);
    stack.initializeCursor(parentNode, nextSibling);
  ...
  constructor(protected doc: Simple.Document) {
  ...
  protected initializeCursor(element: Simple.Element, nextSibling: Option<Simple.Node>) {
